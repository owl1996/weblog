...
This is a long post carrying three separable things, and they can be read
independently:

1. **the certificate** — [three levels of
   guarantee](#three-levels-of-guarantee), only one of which uses a prior, and
   [the hybrid](#the-hybrid) that repairs a step when the test fails;
2. **the measurement** — [does the certificate ever
   fail?](#does-the-certificate-ever-fail), where the answer turns out to be
   less about conflict than about a task starting at its own optimum;
3. **the two-constraint regime** — [where the single test
   breaks](#where-the-single-test-breaks) under heterogeneous priors, an
   [episode with a solver that was
   wrong](#deciding-it-without-trusting-any-solver), and [a reduction that makes
   the problem small](#a-reduction-that-makes-the-problem-small).

Someone who only wants the usable result can stop after (1); (3) is a research
note about a combinatorial problem that happens to be where the construction
leads.
...
The same independence holds of the gate's *noise* model. @prp-level1 reads $f$ as
an opaque non-negative weight, so a gate built for correlated batch noise
certifies through it unchanged — see [Two losses, one
batch](../2026-08-05-correlated-batch-noise/#correctness-is-robust-to-rho-efficiency-is-not),
where getting the correlation wrong is shown to cost efficiency — on shared
batches the test fires more than twice as often with the uncorrelated gate — and
never correctness.
...
to make explicit — either

- **(a)** skip the batch, or
- **(b)** drop the vacuous constraint and proceed on the other task with a
  reduced step.
...
*Left: one of the well-posed instances, symlog on both axes because the
coefficients span twenty orders of magnitude — this is the conditioning that
wrecks the solver, drawn. Colour is the Pareto layer toward $(-\infty,-\infty)$;
the $7{,}924$ grey points are beyond layer $\bar K$ and can be discarded
outright. What is left is the thin staircase along the lower-left frontier of
each conflicting quadrant. Right: the reduction across all $35$ well-posed
instances of this run, against the dominance prefilter $a_i < 0 \vee b_i < 0$
that the $\ell_1$ post already applies — median $5{,}195$ candidates down to
$871$, a factor of $5.7$, or $9.7\%$ of $d$.*
...
and FAIL rate and contains the greedy; `biconstrained.py` collects the
...
certificates;
`make_figure.py` produces the histogram and `make_pareto_figure.py` the layer
figure. Everything that touches real gradients imports the model and data from
the [Adam2D post](../2026-08-03-two-dimensional-adam/)'s `experiment.py`; the
two test scripts are self-contained. A few minutes on CPU each.
