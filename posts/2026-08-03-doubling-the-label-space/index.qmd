...
The fit itself says whether there is anything to read. Comparing the two proxies
as descriptions of the retain population — does the $\mathsf r$-only reading
account for it measurably better than the full model does — answers the question
one would otherwise settle by training something and looking at the result: *is
$s$ actually encoded in this representation, at the layer where the proxy is
attached?* It is a single sum over the dataset, no gradients anywhere, negligible
next to one optimisation step. A negative answer is information rather than a
