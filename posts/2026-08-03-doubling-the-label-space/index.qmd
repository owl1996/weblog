...
@eq-mixture mixes on $X \mid y$; @eq-2c mixes on the posterior. As an identity
the two agree — same components and same weights, and marginalising @eq-2c
returns Bayes on @eq-mixture exactly. They come apart once each side is a
*fitted family* rather than an identity: @eq-mixture has to model $X \mid y$ as a
mixture, @eq-2c models each cell directly, and those are not the same set of
estimable models. The slogan is about what one can fit, not about what one can
write down.
...
settle by training something and looking at the result: *is $s$ actually encoded
in this representation, at the layer where the proxy is attached?* A
non-negative answer is information rather than a
failure: the two readings coincide, so there is no direction to extract, and the
response is to do nothing or to change proxy family or layer — not to push
harder. In the Gaussian family this happens when the subgroup is not separated in
the moments the proxy can see: means only, under a pooled covariance, and means
plus covariances if one moves to QDA. The honest diagnosis is then "there is
nothing to extract here", not "the method missed it". The proxy belongs at an
intermediate layer: early layers have no class geometry yet, and the last ones
are largely redundant with the logits, so they cannot expose a difference the
logits do not already express.
...
- **as a logit processor** — add $\eta \cdot \Delta M(x)$ to the output of an
  existing model: no weights and no gradients, though the proxy does read one
  intermediate activation, so this is not black-box in the sense that a
  logits-only method is;
...
nothing to glue. And because the columns come from a fitted proxy rather than a
training run, the admissibility check above applies *before* anything is applied
at all — including to a correction assembled the other way.
